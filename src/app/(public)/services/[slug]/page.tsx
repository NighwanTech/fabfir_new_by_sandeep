...
async function getServiceBySlug(slug: string) {
  if (!slug || slug === 'default-service') return null;
  try {
...
export async function generateStaticParams() {
  try {
    ...
    if (baseUrl) {
      const res = await fetch(`${baseUrl}/api/services?public=true`, {
        signal: AbortSignal.timeout(3000)
      });
      if (res.ok) {
        const json = await res.json();
    ...
    // Ignore build-time network errors and fallback to static items
  }
  ...
  if (params.length === 0) {
    return [{ slug: 'strength-conditioning' }];
  }
